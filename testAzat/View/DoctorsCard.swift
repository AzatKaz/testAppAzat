        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        HStack {
                            Image(.appClock)
                                ...
                                .clipped()
                            switch model.totalExperience {
                            case 0:
                                Text("Опыт работы: без опыта")
                                    .font(.system(size: 14, weight: .thin))
                            case 1:
                                Text("Опыт работы: \(model.totalExperience) год")
                                    .font(.system(size: 14, weight: .thin))
                            case 2...4:
                                Text("Опыт работы: \(model.totalExperience) года")
                                    .font(.system(size: 14, weight: .thin))
                            default:
                                Text("Опыт работы: \(model.totalExperience) лет")
                                    .font(.system(size: 14, weight: .thin))
                            }
                        }
                    ...
                    VStack(spacing: 24) {
                        NavigationLink {
                            PriceView(textChatPrice: model.text_chat_price, videoChatPrice: model.video_chat_price, hospitalPrice: model.hospital_price)
                        } label: {
                            HStack {
